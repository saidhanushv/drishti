...
class DataLoader:
    ...
    def __init__(self, csv_path: str, config: Config = Config):
        ...
        # Ensure SSL verification is disabled for environments with custom certs
    ...
    def create_duckdb(self) -> duckdb.DuckDBPyConnection:
        ...
        self.conn.execute(f"""
            ...
            SET Quarter = CASE
                WHEN EXTRACT(WEEK FROM TRY_CAST({self.config.WEEK_COLUMN} AS DATE)) BETWEEN 1 AND 13 THEN 'Q1'
                WHEN EXTRACT(WEEK FROM TRY_CAST({self.config.WEEK_COLUMN} AS DATE)) BETWEEN 14 AND 26 THEN 'Q2'
                WHEN EXTRACT(WEEK FROM TRY_CAST({self.config.WEEK_COLUMN} AS DATE)) BETWEEN 27 AND 39 THEN 'Q3'
                WHEN EXTRACT(WEEK FROM TRY_CAST({self.config.WEEK_COLUMN} AS DATE)) BETWEEN 40 AND 52 THEN 'Q4'
                ELSE 'Unknown'
            END
        """)
    ...
    def _prepare_metadata(self, row: pd.Series, index: int) -> Dict:
        ...
        if self.config.WEEK_COLUMN in row.index:
            ...
            if not pd.isna(week_value):
                ...
                dt_val = pd.to_datetime(week_value, errors="coerce")
